...
class UraAtivaEstatistica extends UraAtiva {
		...
		try{
			...
			switch ($opcao){
				...
				case self::ESTATISTICA_NAO_RESPONDEU :
					...
					//Reenvia ao discador
					$telefones = $this->dao->buscarTelefones($contrato);
					$contatos[$vegoid] = $telefones;
					$this->dao->enviarDiscador($contatos);

					//Notifica o Discador
					$this->notificarDiscador($this->dao);
	...
	public function informacoesAdicionais(UraAtivaParamVO $param) {
		...
		if($this->dao->isGravaLogAtendimento){
			...
			$conteudoLog = 	date("d/m/Y H:i") . " | "  . $connumero . " | " . $conclioid . " | " . $cliNome . " | ";
			$conteudoLog .= $param->codigoIdentificador . "#" . $param->idTelefoneContato . "#" . $param->telefoneContato . " | ";
	...
	public function montarHistoricoInsucesso($insucesso, $motivo){
		...
		*/
		 ...
		 * Localizacao
		*/
		$localizacao = $this->dao->buscarUltimaLocalizaoVeiculo($insucesso['contatoExterno']);
	...
	public function tratarInsucessos($insucessos) {
		...
		*/
		if(!in_array($Contrato->conegaoid, $arrAcao)){
            ...
            $this->continuarTratarInsucesos($insucessos, $Cliente, $Contrato, $vegoid);
		...
		*/
		...
		//Array para gravar Log do atendimento
	 ...
	 */
	public function continuarTratarInsucesos($insucessos, $Cliente, $Contrato, $vegoid){

		$enviou = false;
        $listaContratoSimilar = array();

		#Historico contrato
		$observacao = $this->montarHistoricoInsucesso($insucessos, 'Contato sem sucesso');

        $listaContratoSimilar = $this->dao->buscarContratosSimilares($Contrato->connumero);

        //Replica o histórico para todos os contratos similares (mesmo cliente, status, tipo e modalidade)
        foreach($listaContratoSimilar as $contratoSimilar){

           $this->dao->inserirHistoricoContrato($contratoSimilar, $observacao);
