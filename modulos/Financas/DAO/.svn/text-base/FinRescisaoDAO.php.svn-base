...
class FinRescisaoDAO
{
    ...
    public function findContratosNovaRescisao($filters)
    {
        ...
        $sql = "SELECT
                  ...
                  , conprazo_contrato
                  --, diferenca_meses(
                  --      condt_ini_vigencia::text
                  --    , '{$filters['resmfax']}'::text) AS meses_decorridos
     ...
     * @return mixed
     */
    public function finalizarRescisao($arr, $arrRescisoesBaixa)
    {
        ...
        {           
            ...
            if(!empty($arrRescisoesBaixa)) {
                ...
                foreach($arrRescisoesBaixa as $key => $arrRescisaoBaixa) {
                    ...
                    foreach($arrRescisaoBaixa as $rescisaoBaixa) {
                        ...
                        if(!empty($rescisaoBaixa)) {

                            $resbaititoid = $rescisaoBaixa['resbaititoid'];
                            $resbaitipo = "'" . $rescisaoBaixa['resbaitipo'] . "'";  
                            $resbaivl_titulo = $rescisaoBaixa['resbaivl_titulo'];

                            $sql = "INSERT INTO rescisao_baixa (resbaititoid, resbairesmoid, resbaitipo, resbaivl_titulo) 
                                    VALUES ($resbaititoid, $resmoid, $resbaitipo, $resbaivl_titulo)";
            ...
            if(!empty($arr['arrContratos'])){

                foreach($arr['arrContratos'] as $contrato) {
                    $rescconoid             = $contrato['termo'];
                    $rescmeses              = $contrato['mesesRestantes'];
                    $resmstatus             = $_POST['resmstatus'];
                    $rescperc_multa         = $contrato['percentualMulta'];
                    $rescvl_monitoramento   = $contrato['totalDiferencaIndevido'];
                    $rescfax                = $contrato['dataRescisao'];
                    $rescvl_locacao         = $contrato['valorMultaMensalidadeFaltante'];
                    $rescperc_multa_locacao = $contrato['percentualMulta'];
            ...
            if($arr['multas_retirada']) {

                foreach($arr['multas_retirada'] as $retiradaEquipamento) {
                    $rescrcobrar = "'t'";
                    $rescrconoid = $retiradaEquipamento['contrato'];
                    $rescrobr_servico = $this->getObroidByObrobrigacao(utf8_decode($retiradaEquipamento['item']));
            ...
            if($arr['multas_nao_retirada']) {

                foreach($arr['multas_nao_retirada'] as $naoRetiradaEquipamento) {
                    $rescrcobrar = "'t'";
                    $rescrconoid = $naoRetiradaEquipamento['contrato'];
                    $rescrobr_servico = $this->getObroidByObrobrigacao(utf8_decode($naoRetiradaEquipamento['item']));
    ...
    public function gerarNotaFiscalRescisao() {
        ...
        if(!empty($_POST['retiradaEquipamentos'])) {

            foreach($_POST['retiradaEquipamentos'] as $retiradaEquipamento) {
        ...
        if(!empty($_POST['arrContratos'])) {
            ...
            foreach($_POST['arrContratos'] as $contrato) {
                ...
                $notaFiscalItemParams['nficonoid']  = $contrato['termo'];
                $notaFiscalItemParams['nfioid']     = $this->getNextSequenceVal('nota_fiscal_item_nfioid_seq');
                ...
                $notaFiscalItemParams['nfivl_item'] = $contrato['totalMensalidadeEquipamento'];
                $notaFiscalItemParams['nfioid']     = $this->getNextSequenceVal('nota_fiscal_item_nfioid_seq');
                $this->insertNotaFiscalItem($notaFiscalItemParams);
                ...
                $notaFiscalItemParams['nfivl_item'] = $contrato['valorMultaMensalidade'];
                $notaFiscalItemParams['nfioid']     = $this->getNextSequenceVal('nota_fiscal_item_nfioid_seq');
                $this->insertNotaFiscalItem($notaFiscalItemParams);
                ...
                $notaFiscalItemParams['nfivl_item'] = $contrato['valorPagoIndevidoMonitoramentoTotal'] + $contrato['totalMensalidadeIndevido'];
                $notaFiscalItemParams['nfioid']     = $this->getNextSequenceVal('nota_fiscal_item_nfioid_seq');
                $this->insertNotaFiscalItem($notaFiscalItemParams);
            }
        }
        ...
        // [START][ORGMKTOTVS-1986] - Leandro Corso 
        if (!INTEGRACAO) {
            $tituloID                   = $this->getNextSequenceVal('titulo_titoid_seq');
            $tituloNotaFiscalID         = $notaFiscalID;
            $tituloClienteID            = $notaFiscalClienteID; 
            $tituloNumeroParcela        = 1;
            $tituloValorTitulo          = $notaFiscalValorTotal;
            ...
            $tituloTarifaUnica          = "'f'";
            $tituloCobrancaTerceirizada = "'f'";
            $tituloRecebimento          = "'CAIXA'";
            $tituloObsHistorico         = "'.'";
            $tituloDataReferencia       = $notaFiscalDataReferencia;
            $tituloDataVencimento       = $notaFiscalDataVencimento;
            $tituloDataEmissao          = date("'Y-m-d'");
            ...
            $boleto->setValorFace($tituloValorTitulo);
            $boleto->setValorNominal($tituloValorTitulo);
    ...
    public function getDescricaoObroid($obroid) {
        ...
        $sql = "SELECT obrobrigacao FROM obrigacao_financeira WHERE obroid = $obroid";
    ...
    public function enviarEmail() {
        ...
        //$boleto = BoletoRegistradoModel::getUltimoBoletoValido($_POST['titoid']);
        //$boletoHTML = $boleto->imprimir()->dados;
        //$attachments['boletoPDF'] = RoteadorBoleto::getArquivoBoleto($_POST['titoid'], '', $boletoHTML);
        //$attachments['boletoPDF'] = file_get_contents($attachments['boletoPDF']);
        // // FIM BOLETO MULTA

       /* if(isset($_POST['idsbaixa'])){

            $boletosBaixas = explode(",", $_POST['idsbaixa']);
            $arrAttachment = array();

            foreach($boletosBaixas as $boletoBaixa){

                $boletoBaixa = (int)$boletoBaixa;

                $boletoValido = BoletoRegistradoModel::getUltimoBoletoValido($boletoBaixa);
                $boletoValidoHTML = $boletoValido->imprimir()->dados;
                $attachmentFile = RoteadorBoleto::getArquivoBoleto($boletoBaixa, '', $boletoValidoHTML);
                $attachmentFile = file_get_contents($attachmentFile);

                $arrAttachment[] = array(
                    'filename' => "boleto_$boletoBaixa.pdf",
                    'file' => $attachmentFile
                );
                
                unset($boletoValidoHTML);
