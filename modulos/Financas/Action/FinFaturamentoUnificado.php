...
class FinFaturamentoUnificado extends FinFaturamentoUnificadoDAO {
	...
	public function verificarProcesso($finalizado) {
		try {
    		...
    		return array(
					...
					$mail->Subject = "SASCAR - Falha na Geração de Resumo";
                     
					echo $msg = "O processo de geração de resumo iniciado às ".$param['exfdt_inicio']." falhou com a seguinte descrição de erro: ".$e->getMessage();
    ...
    public function gerarResumo() {
    	...
    	try {
			...
			echo
			$msg = "O processo de geração de resumo iniciado às ".$param['inicio']." foi finalizado com sucesso às ".$param['termino'];
			...
			echo
			$msg = "O processo de geração de resumo iniciado às ".$param['inicio']." falhou com a seguinte descrição de erro: ".$e->getMessage();
    ...
    public function gerarFaturamento() {
		...
		try {
			...
			echo
			$msg = "O processo de geração de faturamento iniciado às ".$param['exfdt_inicio']." falhou com a seguinte descrição de erro: ".$e->getMessage();
    ...
    public function getINPC() {
    	try {
    		...
    		if ($inpvl_referencia  == '') {
    			throw new exception("INPC não cadastrado, deseja prosseguir?", 1);
    		}
