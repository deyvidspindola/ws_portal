...
class FinRetornoCobrancaRegistrada {
    ...
    public function importarDados(){
 	    ...
 	    try{
	    	...
	    	$arrBancos = array('399','341','664'); //HSBC - Itaú - Bradesco (237)
           ...
           while (!feof($arquivo)) {
           	...
           	if($cod_registro == '1'){
           		...
           		if( $forccfbbanco == 341 ){
           			...
           			// Se não for do tipo T (titulo) ou E (evento) gera erro
           			//if($tipo_registro != 'T' && $tipo_registro != 'E'){
           			//    throw new exception('Erro: Linha '.$linha.' com código de OID inválido');
           			//}
           		...
           		//Foi criado o código 664 para identificação no Bradesco no sistema, pois
           		//a conta existente (para o banco 237) não receberá os movimentos da nova conta criada pelo bradesco
           		...
           		if (!is_array($result)){
           			...
           			if (!is_array($result)) {
           				...
           				if(trim($titoid) != '') {
           					...
           					/*
           					 * MANTIS 3707:
           					* Caso a consulta a título_kernel retorne mais que 1 registro
           					* tentar bater o valor do título com o valor pago.
           					*/
           							...
           							if (!is_array($result)){
           								...
           								if (!is_array($result)) {
           									...
           									if (!is_array($result)) {
           										...
           										//verifica se é um título unificado somente para o sistema da SASCAR (schema PUBLIC)
           										//if($this->sistema == 'PUBLIC'){
           										
	           										###  PESQUISA SE É UM TÍTULO PAI NA TABELA titulo_consolidado
           										...
           										if (is_array($result)) {
           											...
           											/**
           											 * MANTIS 3383
           											 * Incidente Nº 18560 / Não está gerando NF quando há pagamento de boleto seco
           											*/
           							...
           							if( in_array($cod_ocorrencia,$arrCodBaixa) and !empty($titoid)) {
           								...
           								*/
           								...
           								if($resul == 2){
           									...
           									$mErroProcessamento[] = 'Erro linha '.$linha.': Falha ao executar função ->  emite_boleto_recup_ativo: <br/>'.
           									                         'Título = '. $tit_numero_registro_banco_local.' <br/>'.
           		...
           		if( in_array($cod_ocorrencia,$arrCodBaixa) && $titoid > 0) {
           			...
           			if (is_array($res)) {
           				...
           				$retorno = retirarClienteOperacoes ($titoid, $titclioid, $valor_credito_titulo, $cd_usuario, "Baixa cobrança registrada");
           			}
           ...
           if($qtde_atualizado_cod > 0){
           		$msg_aux .= ' Total de <b>'.$qtde_atualizado_cod .'</b> registros atualizados com código de retorno';
           }
    ...
    */
    ...
    public function montarHtmlTitulosComBaixa($mTitulosJaBaixados,$nTotal_Ja_Baixados){
    	...
    	$msg_aux .= '<table width="100%" class="tableMoldura">';
    	$msg_aux .= '<tr><td align=left colspan=5>Os seguintes títulos não puderam ser baixados pois já possuem data de baixa.</td></tr>';
    	$msg_aux .= '<tr class="tableSubTitulo">';
    	$msg_aux .= '<td align=left><b>Cliente</b></td>';
    	$msg_aux .= '<td align=center><b>Cód. Título</b></td>';
    	$msg_aux .= '<td align=center><b>NF</b></td>';
      		...
      		if(empty($emailUsuarioProcesso[0]['usuemail'])){
      			...
      			if($status){
      				...
      				if($tipo == 'pararExecucao'){
      					...
      					$corpo_email = " Prezado usuário o processamento do arquivo de retorno do banco que você iniciou às  $dadosProcesso->inicio_hora do dia  $dadosProcesso->inicio_data ,
    					                 foi parado pelo usuário  [ $nomeUsuarioProcesso ] e o arquivo foi disponibilizado para processamento novamente. ";
