	...
	public function navegacao(UraAtivaParamVO $param) {
		...
		try {
			...
			*/
			...
			if (empty($atcoid)) {
				throw new Exception('0230'); // Não existe atendimento para o Pânico informado
			}
			/*
            ...
            /*
             * Gravação de arquivo log
             */
			if($this->dao->isGravaLogAtendimento){

                $dataContato = 'Sem Data/Hora';

				$idContato = $this->dao->buscarContatoDiscadorEspecifico($atcoid);
                /*
                 * Busca data de contato no discador
                 */
				$retornoDB2 = $this->dao->buscarInsucessos($idContato, $panico_pendente->veioid);
				if(!empty($retornoDB2)){
					$dataContato = isset($retornoDB2['chamada']) ? trim($retornoDB2['chamada']) : '';
				}

				$conteudoLog = $Contrato->connumero . " | ";
                $conteudoLog .= $Contrato->conclioid . " | ";
				$conteudoLog .=  $param->opcaoSelecionada .  " | ";
                $conteudoLog .= $param->telefoneContato . " | ";
                $conteudoLog .= $dataContato . " | ";
			...
			if($contrato->connumero > 0){
                ...
                if($this->dao->isGravaLogAtendimento) {
                    $this->logAtendimento = array(
                            ...
                            'conveioid' => $contrato->conveioid,
                            'panico'    => $papoid,
                            'motivo'    => 'Pânico pendentes a mais de uma hora: Precisa Verificar e Retorna.'
                    );
                }
