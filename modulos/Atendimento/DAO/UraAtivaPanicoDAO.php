...
class UraAtivaPanicoDAO extends UraAtivaDAO {
	...
	/**
	 * Flag de tratamento de insucessos para contatos com retorno do WS de navegação com o motivo PANICO_FALAR_ATENDENTE
	 * @var boolean
	 */
	...
	/**
	 * Busca pelos contatos com atendimentos pendentes pelo motivo PANICO_FALAR_ATENDENTE há mais de 20 minutos
	 *
	 * @param int $atmoid
	 * @return array
	 */
	public function buscarContatosFalarAtendente($atmoid){
		...
		foreach($contratos as $contrato){
			...
			//verifica se o registro relamente não existe na tabela auxiliar
	...
	protected function descartar(UraAtivaContratoVO $panico) {
        ...
        if($this->desconsiderarTipoPanicoEspecifico($panico->codigo)) {
            ...
            $obsLog = "Tipo Violação GPS Bateria";
        }
    ...
    /**
     *  Verifica se o pânico em questão é de um tipo especifico de panico
     *
     * @param int $papoid
     * @return boolean
     */
...
echo $sql;
    ...
    /**
     * Aciona os métodos responsáveis pela limpeza de pânciso que devem ser ignorados
     * @return type
     */
    ...
    /**
     * Remove das tabelas auxiliares os contatos referentes a pânicos que
     * não necessitam mais do atendimento pela URA
     */
    private function limparPanicosAtendidos() {
        ...
        /*
         * Buscar os pânicos da tabela auxiliar
         */
        $sql = "
            SELECT
	...
	/**
	 * Busca por atendimentos de Pânico pendentes há mais de 20 minutos com motivo Falar com Atendente
	 * @param int $atmoid
	 * @return objeto $atendimentos
	 */
