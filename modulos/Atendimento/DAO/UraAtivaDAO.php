...
abstract class UraAtivaDAO extends AtendimentoDAO {
	...
	public function enviarDiscador($contatos) {
		...
		foreach ($contatos as $codigo => $telefones) {
			...
			foreach ($telefones as $contato) {
               ...
               //Grava Array com log do envio / reenvio
               if($this->isGravaLogAtendimento) {
                    $clinome = $this->getNomeCliente($contrato->conclioid);

                    if($this->nomeCampanha == 'assistencia') {
                         $arrLogEnvio[] = $contrato->conclioid . " | " . $clinome . " | " . $this->insertDiscador;
                    }
                    else{
                        $arrLogEnvio[] = $contato->connumero . " | " . $contrato->conclioid . " | " . $clinome . " | " . $this->insertDiscador;
                    }
	...
	protected function inserirDiscador(UraAtivaContatoVO $contato, $codigoIdentificador, UraAtivaContratoVO $Contrato) {
        ...
        //Para uso do arquivo de log
        $this->insertDiscador = str_replace("CALL BXS.V29_INSERE_CONTATO", '' , $sql);
		...
		if($this->tabela_auxiliar_discador == 'contato_discador_ura_assistencia'){
				...
				$sql = "
                    ...
                    VALUES
                        ...
                        ".$Contrato->conclioid.",
                        NOW(),
                        '".implode(",", $this->osCliente[$Contrato->conclioid])."'
                        )";
	...
	protected function isDescartaTipoContrato($conno_tipo, $paramTiposContrato) {
		...
		if (empty($conno_tipo) || count($paramTiposContrato) == 0) {
			return false;
		}
    ...
    public function buscarVeiculoPosicao($filtros) {
        ...
        if (isset($filtros['limite_resultados'])) {
    		$filtro .= " AND ROWNUM <= " . $filtros['limite_resultados'];
    	}
     ...
     */
    public abstract function verificarInsucessoContato($codigo, $telefone);
    ...
    public function buscarInsucessoEspecifico($idContatoExterno, $idTelefoneExterno = '') {
    	...
    	if(empty($this->campanha) || empty($idContatoExterno)){
    		return '';
    	}
    	...
    	$sql = "
				...
				WHERE
    					id_campanha = ". intval($this->campanha) ."
    			AND
    					id_contato_externo = ". intval($idContatoExterno) ."
				AND
    					motivo_encerram != 'Atendida'
    		";

    	if($idTelefoneExterno) {
    		$sql.= "AND id_telefone_externo IN (".$idTelefoneExterno.")";
    	}
