...
class CadDadosEquipamento {
    ...
    public function __construct($dao = null) {
        ...
        // Servidor comandos produção
        $this->servidorComandos = '172.19.14.56';
        if (defined('_SERVIDOR_COMANDOS_')) {
            $this->servidorComandos = _SERVIDOR_COMANDOS_;
        }
        ...
        $this->configDadosEquipamento = array(
            // 'MTC550'       => 'COMMAND_MTC', 
            // 'MTC600'       => 'COMMAND_MTC', 
            'MTC700'       => 'COMMAND_MTC', 
            'MTC700A'      => 'COMMAND_MTC', 
        ...
        $this->configDadosEquipamentoCommands = array(
            'COMMAND_MTC' => '__ESN__ 1 1 1 GPRS M6.GET_SETUP 0 " "__FINALIZADOR____ACAO____FINALIZADOR__',
            
            'COMMAND_LMU' => '__ESN__ 1 1 1 GPRS GETSETUP 0 " "__FINALIZADOR____ACAO____FINALIZADOR__'
        );
        ...
        try {
            ...
            if (isset($this->view->parametros) && 
                ...
                if ($this->verificarFiltro() == false) {
                    ...
                    (isset($this->view->parametros->cliente) && !empty($this->view->parametros->cliente)) &&
                    (isset($this->view->parametros->clioid) && empty($this->view->parametros->clioid))
                ) {
    ...
    private function verificarFiltro() {
        $campos = array('placa', 'chassi', 'contrato', 'clioid', 'cliente');
    ...
    private function pesquisar(stdClass $filtros) {
        ...
        $retorno = array();
        $retorno['COMMAND_MTC'] = array();
        $retorno['COMMAND_LMU'] = array();
        $retorno['OUTROS'] = array();

        foreach ($resultadoPesquisa as $key => $equip) {
            if (array_key_exists($equip->eprnome, $this->configDadosEquipamento)) {
                $retorno[$this->configDadosEquipamento[$equip->eprnome]][] = $equip;   
            } else {
            ...
            if (is_dir($diretorio) && is_writable($diretorio)) {
                ...
                if(count($dados['COMMAND_LMU']) > 0) {
                    ...
                    $csvWriter->addLine(array(
                        ...
                        utf8_decode('Eq Versão'),
                        utf8_decode('Eq Projeto'),
                        utf8_decode('Firmware'),
                        utf8_decode('Versão Perfil'),
                        utf8_decode('Versão Teclado'),
                        utf8_decode('Hosted App'),
                        utf8_decode('Modelo ISV'),
                        // utf8_decode('Blackbox'),
                        utf8_decode('Peg Enables'),
                        utf8_decode('Logoff Após 30seg'),
                        utf8_decode('App Tempo de Direção'),
                        utf8_decode('Bloqueio Automático Ignição'),
                    ...
                    foreach($dados['COMMAND_LMU'] as $resultado) {
                        ...
                        if (property_exists($dadosEquipamento, $esn)) {
                            array_push(
                                $linha,
                                $dadosEquipamento->$esn->firmware,
                                $dadosEquipamento->$esn->versao_perfil,
                                $dadosEquipamento->$esn->versao_teclado,
                                $dadosEquipamento->$esn->hosted_app,
                                $dadosEquipamento->$esn->modelo_isv,
                                // $dadosEquipamento->$esn->telemetria_segundo_a_segundo,
                                $dadosEquipamento->$esn->peg_enables,
                                $dadosEquipamento->$esn->logoff_30seg,
                                $dadosEquipamento->$esn->app_tempo_direcao,
                                $dadosEquipamento->$esn->bloq_auto_ignicao,
                                $dadosEquipamento->$esn->inbound_url00,
                                $dadosEquipamento->$esn->inbound_port00,
                                $dadosEquipamento->$esn->inbound_url01,
                                $dadosEquipamento->$esn->inbound_port01,
                                $dadosEquipamento->$esn->data_chegada,
                                $dadosEquipamento->$esn->ini_speed_deaccel,
                                $dadosEquipamento->$esn->breack_deaccel
                            );
                        }
    ...
    public function getDadosEquipamento() {
        ...
        try {
            ...
            if (array_key_exists($filtros->projeto, $this->configDadosEquipamento)) {
                ...
                if($key == 'COMMAND_LMU'){
                    $peg_enables_normalizado = base_convert($retorno['advanced_setup']['others']['peg_enables']['@attributes']['val'], 16, 2);

                    $logoff_30seg = (substr($peg_enables_normalizado, -23, 1) == "1" ? "Ativado" : "Desativado");
                    $app_tempo_direcao = (substr($peg_enables_normalizado, -24, 1) == "1" ? "Ativado" : "Desativado");
                    $bloq_auto_ignicao = (substr($peg_enables_normalizado, -25, 1) == "1" ? "Ativado" : "Desativado");

                    $retorno['peg_enables_normalizado']['logoff_30seg'] = $logoff_30seg;
                    $retorno['peg_enables_normalizado']['app_tempo_direcao'] = $app_tempo_direcao;
                    $retorno['peg_enables_normalizado']['bloq_auto_ignicao'] = $bloq_auto_ignicao;
                }
