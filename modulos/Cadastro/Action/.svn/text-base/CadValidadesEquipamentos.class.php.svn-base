<?php
/**
 * @file CadValidadesEquipamentos.class.php
...
/**
 * Action do Cadastro de Validades de Equipamentos
 */
class CadValidadesEquipamentos {
	...
	public function __construct() {		
		...
		global $conn;
		$this->dao = new CadValidadesEquipamentosDAO($conn);
	...
	public function editar($acao = 'editar', $mensagem = '') {	
		...
		if($params['acao'] == 'editar')
			$result = $this->dao->getValidadeEquipamento($params['veqpoid']);		
		else
			$result = $params;	
	...
	public function excluir($acao, $mensagem = '') {
		...
		$resultado = $this->dao->excluirDados($this->veqpoid);
		$this->populaValoresPost(true);
		$this->index('index', '', $resultado);		
	}
	...
	public function view($action, $layoutCompleto = true) {
		if($layoutCompleto)
			include _MODULEDIR_.'Cadastro/View/cad_validades_equipamentos/header.php';
		
		include _MODULEDIR_.'Cadastro/View/cad_validades_equipamentos/'.$action.'.php';
		
		if($layoutCompleto)
			include _MODULEDIR_.'Cadastro/View/cad_validades_equipamentos/footer.php';
	}
