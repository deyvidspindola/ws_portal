...
class CadTiposVeiculo {
    ...
    public function index() {
        ...
        require_once _MODULEDIR_ . "Cadastro/View/cad_tipos_veiculo/index.php";
    }
    ...
    public function cadastrar($parametros = null) {
        ...
        if ($registroGravado){
            ...
            require_once _MODULEDIR_ . "Cadastro/View/cad_tipos_veiculo/cadastrar.php";
        }
    }
    ...
    public function editar() {
        ...
        try {
            ...
            if (isset($parametros->tipvoid) && intval($parametros->tipvoid) > 0) {
                //Realiza o CAST do parametro
                $parametros->tipvoid = (int) $parametros->tipvoid;

                //Pesquisa o registro para edição
                $dados = $this->dao->pesquisarPorID($parametros->tipvoid);
    ...
    private function validarCamposCadastro(stdClass $dados) {
        ...
        if (!isset($dados->tipvcategoria) || trim($dados->tipvcategoria) == '') {
            $camposDestaques[] = array(
                'campo' => 'tipvcategoria'
            );
        }
        if (!isset($dados->tipvdescricao) || trim($dados->tipvdescricao) == '') {
            $camposDestaques[] = array(
                'campo' => 'tipvdescricao'
            );
        }
        ...
        if (!isset($dados->tipvdescricao) || strlen(trim($dados->tipvdescricao)) < 3) {
            $camposDestaquesTamanho[] = array(
                'campo' => 'tipvdescricao'
            );
        }
