...
class CadContratoServicos {
	...
	public function chamaAtivacao($params){		
		...
		// Verifica se status for aguardando ativação ou ativado então não entra
		if($this->dao->getStatusAtivacao($params['serial']) == 'N'){
			...
			if($retorno->codigo == '0001'){				
				...
				// Chama WS que vericica se o equipamento está disponível para ativação
				$retorno = $this->disponivel($api_key,$params['serial']);

				// Entra aqui caso esteja pronto para ser ativado
				if($retorno['status'] == 'sucesso'){
					
					// Verifica se o equipamento já está em processo de ativação em outro contrato
					if($this->dao->verificaExisteProcessoAtivacao($params['serial'],$params['consoid']) == false){
						...
						if($retorno->codigo == '0005'){
							...
							$array['status'] 	= "Error";
							$array['mensagem'] 	= utf8_encode("N&atilde;o foi possível ativar o equipamento");
							$array['codstatus'] = 1804;// Não foi possível ativar o equipamento
						...
						$array['codstatus'] = 1803;// Este equipamento já está em processo de ativação em outro contrato
					}
					...
					$array['codstatus'] = 1802;// Equipamento não está disponível para ativação
				...
				$array['codstatus'] = 1801;// Instalador não autorizado para instalar o equipamento
	...
	/**
	 * Pega os dados do contrato, veiculo e equipamento para chamar a ativação
	 */
	public function chamaAtivacaoEquipamentoPrincipal($params){	    
	    ...
	    // caso o RF esteja importado na base o mantém
	    ...
	    if(!empty($params['placa']) && !empty($params['cpf']) && !empty($params['local']) && !empty($params['rf'])){
			...
			if(empty($params['local'])){
				$error_msg .= 'Local de Instalação n&atilde;o encontrado. ';
			}
    ...
    /**
	 * Verifica se existe solicitação, o status é R e faz mais de X minutos
	 */
	...
	/**
	 * Manda sinal para o WS para verificar a posição
	 */
	public function posicao($api_key,$serial,$method = "GET"){
		...
		if($ccid == 0){
			$array['status'] 	= "Error";
			$array['mensagem'] 	= "CCID não encontrado";
	...
	public function disponivel($api_key,$serial){
		...
		if($resultado->codigo == '0003'){
			...
			//TODO : fazer validação quando ws retornar numero de posicoes
	...
	public function autenticarInstalador($cpf){
	    ...
	    $cpf = $this->getCPF(preg_replace("/[^0-9]/","",$cpf)); // mantém somente numeros da string e completa com zeros a esquerda
		$params['cpf']	= $this->mascaraString('###.###.###-##',$cpf); // adiciona mascara de formatação CPF
	...
	/**
	 * Pega parametros para ativação através do serial
	 * @param int $serial
	 * @return Array $array
	 */
	...
	/**
	 * Valida se equipamento é CargoTracck
	 * @param int $serial
	 * @return boolean
	 */
	...
	/**
	 * Valida se contrato serviço é CargoTracck
	 * @param int $serial
	 * @return boolean
	 */
	...
	/**
	* Método para validar status da linha
	**/
	public function validaStatusLinha($equno_serie)
	{
		...
		$mensagens = array(
            'habilitarLinha' => "A linha vinculada a esse equipamento encontra-se com o status %s. \n\nVocê não poderá vincular esse equipamento a outro contrato enquanto a linha não estiver habilitada.",
        );

		$regras = array(
			//Para os status abaixo, trava a migração e mostra alerta solicitando abertura de ASM para reativação da linha
            ...
            12 => array ('trava' => true, 'alerta' => $mensagens['habilitarLinha']), //Disponível para troca de chip
